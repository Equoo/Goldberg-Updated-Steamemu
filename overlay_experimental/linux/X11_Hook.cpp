...
bool X11_Hook::StartHook(std::function<bool(bool)>& _key_combination_callback, std::set<ingame_overlay::ToggleKey> const& toggle_keys)
{
    if (!_Hooked)
    {
        ...
        struct {
            ...
            { reinterpret_cast<void**>(&XEventsQueued), reinterpret_cast<void*>(&X11_Hook::MyXEventsQueued), "XEventsQueued" },
            { reinterpret_cast<void**>(&XPending), reinterpret_cast<void*>(&X11_Hook::MyXPending), "XPending" },
        };
